                ...
                {currentUser && (detailedPart.sellerId === currentUser.id || currentUser.email === "wwwautoparts2@gmail.com") ? (
                  ...
                  <>
                    <button
                      ...
                      className={`w-full flex flex-row items-center justify-center gap-2 py-3.5 rounded-xl font-black text-xs uppercase cursor-pointer transition-colors shadow-sm ${
                        detailedPart.sold ? "bg-slate-100 text-slate-400 cursor-not-allowed" : "bg-teal-600 hover:bg-teal-700 text-white"
                      }`}
                      id="inapp-chat-btn"
                    >
                      <MessageSquare size={16} className={detailedPart.sold ? "text-slate-400" : "text-white"} />
                      <span className={`font-black text-xs uppercase ${detailedPart.sold ? "text-slate-400" : "text-white"}`}>
                        {detailedPart.sold ? t("soldOut") : "Contact Seller via In-App Chat"}
                      </span>
                    </button>
                  </>
                )}
