...
export default function ProfileScreen({ 
  ...
  const handlePhotoFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (evt) => {
      if (evt.target?.result) {
        setPreviewPhotoUrl(evt.target.result as string);
      }
    };
  ...
  const handleSaveProfile = async (e: React.FormEvent) => {
    ...
    try {
      let finalPhotoUrl = currentUser.photoURL || currentUser.profilePhoto || "";

      if (previewPhotoUrl && (previewPhotoUrl.startsWith("data:") || previewPhotoUrl.startsWith("blob:"))) {
        finalPhotoUrl = await uploadProductImage(previewPhotoUrl);
          ...
          <div className="bg-[#0B1220] text-white px-4 py-5 relative shadow-md border-b border-[#18233C]" id="profile-header-olx">
            <div className="flex items-center justify-between gap-3">
              <div className="flex items-center gap-3.5 min-w-0">
                <div className="w-14 h-14 rounded-full overflow-hidden bg-slate-800 border-2 border-blue-500/40 shrink-0 shadow-md flex items-center justify-center font-black text-white text-xl uppercase">
                  {currentUser.photoURL || currentUser.profilePhoto ? (
                    ...
                    <div className="w-full h-full bg-gradient-to-tr from-blue-600 to-indigo-600 flex items-center justify-center font-black text-white text-lg">
          ...
          <div className="p-4 space-y-4 max-w-4xl mx-auto w-full pb-28">
            ...
            <div className="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-[0_2px_12px_rgba(11,18,32,0.04)] flex flex-col sm:flex-row items-center sm:items-start justify-between gap-4 text-center sm:text-left">
              <div className="flex flex-col sm:flex-row items-center gap-4">
                <div className="w-20 h-20 rounded-full overflow-hidden bg-slate-100 border-2 border-slate-200 shrink-0 shadow-sm flex items-center justify-center font-black text-slate-700 text-2xl uppercase">
                  {currentUser.photoURL || currentUser.profilePhoto ? (
                    <img 
                      src={currentUser.photoURL || currentUser.profilePhoto} 
                      alt={currentUser.name || "User"} 
                    ...
                    <div className="w-full h-full bg-gradient-to-tr from-indigo-600 to-sky-600 flex items-center justify-center font-black text-white text-2xl">
                      {currentUser.name ? currentUser.name.substring(0, 2) : "ME"}
                    </div>
                  )}
                </div>
          ...
          <form onSubmit={handleSaveProfile} className="p-4 space-y-5 max-w-lg mx-auto w-full pb-28">
            ...
            <div className="bg-white p-6 rounded-2xl border border-slate-200/80 shadow-[0_2px_12px_rgba(11,18,32,0.04)] space-y-6">
              ...
              <div className="flex flex-col items-center justify-center text-center space-y-3">
                ...
                >
                  <div className="w-full h-full rounded-full overflow-hidden bg-slate-100 border-2 border-indigo-500/30 shadow-md flex items-center justify-center font-black text-slate-700 text-3xl uppercase transition-all group-hover:opacity-90">
                    {previewPhotoUrl ? (
                      <img 
                        src={previewPhotoUrl} 
                        alt="Profile Avatar" 
                      ...
                      <div className="w-full h-full bg-gradient-to-tr from-indigo-600 to-sky-600 flex items-center justify-center font-black text-white text-3xl">
                        {editName ? editName.substring(0, 2) : "ME"}
                      </div>
                    )}
                  </div>

                  <div className="absolute bottom-0 right-0 p-2.5 bg-blue-600 text-white rounded-full shadow-lg border-2 border-white group-hover:bg-blue-700 transition-colors">
                    <Camera size={16} />
                  </div>
                </div>
                ...
                <input 
                  ...
                  className="hidden"
                  id="input-profile-photo-file"
                />

                <p className="text-[10px] font-bold text-slate-400">
                  Tap avatar to upload a new photo (Cloudinary)
                </p>
              </div>
